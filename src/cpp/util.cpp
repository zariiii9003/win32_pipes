...
auto AnsiToUtf8(const char *ansiString) -> std::string
{
    ...
    MultiByteToWideChar(CP_ACP, 0, ansiString, -1, &wideString[0], numWideChars);

    int numUtf8Bytes = WideCharToMultiByte(CP_UTF8, 0, wideString.c_str(), -1, NULL, 0, NULL, NULL);
    if (numUtf8Bytes == 0) {
        // Handle error
        return "";
    }
    ...
    WideCharToMultiByte(CP_UTF8, 0, wideString.c_str(), -1, &utf8String[0], numUtf8Bytes, NULL, NULL);
